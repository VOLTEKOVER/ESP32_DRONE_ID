...
- [ ] **BLE TX power impl** (`ble_tx.c:195`) — `ble_tx_set_power()` ignores `dbm` param
- [ ] **OTA timeout** (`rid_ota.c:192-194`) — infinite loop if no upload
...
- [ ] **Startup delay Kconfig** — configurable boot delay
...
|------|-------|--------|-------|
| `main.c` | 107 | ✅ OK | Entry point, splash |
...
| File | Lines | Status | Notes |
|------|-------|--------|-------|
| `CMakeLists.txt` | 49 | ✅ OK | 23 src files, REQUIRES |
| `Kconfig.projbuild` | 27 | ✅ OK | LED GPIO config |
...
| `esp_remote_id.c` | 543 | 🟡 WATCHDOG | Needs task_wdt + absolute GPS timeout |
| `web_config.c` | 735 | 🔴 SECURITY | JSON parser, base64, rate limiting |
| `cli.c` | 317 | 🟡 NEEDS | Missing `config set` + history |
| `wifi_tx.c` | 204 | 🟡 DEDUP | Deduplicate populate_uas_data |
| `wifi.c` | 614 | ✅ OK | Intel ODID frame builder |
| `ble_tx.c` | 198 | 🟡 POWER | ble_tx_set_power() ignores dbm |
| `mavlink_parser.c` | 276 | 🟡 SUBMSG | Empty submsg switch |
...
- Templates ✅ bug_report, feature_request, PR checklist

### `docs/` — GitHub Pages

| File | Lines | Status | Notes |
|------|-------|--------|-------|
| `index.html` | ~913 | ✅ OK | Landing + Quick Start |
| `guide.html` | ~1864 | ✅ OK | Technical wiki |
...
### Root Files
- `README.md` (136) ✅ Protocol table, XIAO C6 recommended
...
|------|---------|--------|--------|
| P0 | Security fixes (JSON, base64, rate limit, watchdog) | ~3d | 🔜 Next |
...
| P3 | Devcontainer + Makefile | ~1d | Port from peinser |
...
## Port Sources
- **peinser/esp-remoteid** — Ed25519, OTA, DroneCAN, MAVLink features, flash encryption, WS2812, GPIO lighting, devcontainer. Most ported; missing flash encryption, Kconfig items, devcontainer.
