...
Inside `esp_rid_init()` (`esp_remote_id.c:138-236`):

| # | Step | Where | Note |
|---|------|-------|------|
| 1 | `nvs_storage_init` (erase+reinit if corrupted) | `nvs_storage.c:10-18` | `ESP_ERROR_CHECK`; crash here = NVS partition broken |
| 2 | `default_config(&g_config)` | `esp_remote_id.c:143` | defaults: proto AUTO, baud 57600, pins 17/18, TX=WiFi beacon, bcast_powerup=1, start_delay 10 s, region AUTO |
| 3 | `nvs_storage_load(&g_config)` | `esp_remote_id.c:144` | persisted fields only (see §6.12); `region` key loaded here |
| 3b | `active_standard` / `standard_fallback` bound from region | `esp_remote_id.c:147-148` | `rid_output_active_standard()` + `rid_output_has_encoder()`; also re-bound in `esp_rid_set_config` (`:244-245`) |
| 4 | `rid_ota_check_and_run(&g_config)` | `esp_remote_id.c:151`, `rid_ota.c:304-338` | **if OTA GPIO pulled low → enters OTA mode and loops forever; the rest of boot never runs** |
| 5 | `g_lock = xSemaphoreCreateMutex()` | `esp_remote_id.c:153` | |
| 6 | `memset(&g_state, 0)` | `esp_remote_id.c:155` | |
| 7 | startup delay `start_delay_ms` (default 10 s) | `esp_remote_id.c:158-161` | `vTaskDelay`, blocks main |
| 8 | boot baud = 115200 (AUTO) else `baud_rate` | `esp_remote_id.c:164-165` | |
| 9 | `protocol_detect_init` → UART driver (RX 256 B) | `esp_remote_id.c:166`, `protocol_detect.c:18-40` | |
| 10 | `nmea/msp/mavlink_parser_init` + `mavlink_parser_set_sysid_filter` | `esp_remote_id.c:167-170` | all three share UART1 |
| 11 | `esp_netif_init()` | `esp_remote_id.c:172` | |
| 12 | `wifi_tx_init` (event loop, AP, power, MAC) | `esp_remote_id.c:174`, `wifi_tx.c:66-124` | |
| 13 | `ble_tx_init` (BT controller + Bluedroid) | `esp_remote_id.c:175`, `ble_tx.c:124-141` | |
| 14 | `ble_tx_set_power(9)` | `esp_remote_id.c:178` | |
| 15 | if `options & (bit6\|bit7)` or `mavlink_usb_enable` → `rid_mavlink_tx_init` + create task | `esp_remote_id.c:180-188` | |
| 16 | if `options & AUTH_ED25519` → `rid_auth_init(auth_private_key)`; `g_state.auth_enabled = rid_auth_enabled()` | `esp_remote_id.c:191-194` | key only parsed here, at boot |
| 17 | `led_ws2812_init` | `esp_remote_id.c:197` | RMT |
| 18 | if any `lighting_pins[i] >= 0` → `rid_lighting_init` | `esp_remote_id.c:200-206` | runs once, not per pin |
| 19 | if both dronecan pins set → `rid_dronecan_init` | `esp_remote_id.c:209-212` | TWAI install+start |
| 20 | if `mavlink_usb_enable` → `rid_mavlink_usb_init` | `esp_remote_id.c:215-217` | may fail if console owns the UART |
| 21 | if `uas_id=="ESP32-RID-001"` or `operator_id=="OP-UNKNOWN"` → MAC-based IDs + NVS save | `esp_remote_id.c:219-226` | |
| 22 | `led_status_reconfigure` | `esp_remote_id.c:228` | first LED init (LEDC) |
| 23 | `web_config_init(webserver_en)` | `esp_remote_id.c:229`, `web_config.c:746-775` | starts HTTP server, 10 handlers |
| 24 | `cli_init()` → creates `cli_task` | `esp_remote_id.c:231`, `cli.c:413-417` | |
| 25 | `rid_kalman_init(&g_kalman)` | `esp_remote_id.c:233` | |
| 26 | **`esp_rid_start()`** (separate call from main) creates `rid_task` | `esp_remote_id.c:679-685`, `main/main.c:106` | |
...
| `rid_task` | `esp_remote_id.c:683` | 4096 | 5 | core 100 ms loop (§5), subscribed to WDT |
| `cli_task` | `cli.c:415` | 4096 | 5 | reads UART0 stdin, executes commands (§6.11) |
| `rid_mavlink_tx` | `esp_remote_id.c:184` | 2048 | 3 | heartbeat 1 s + ODID_SYSTEM 6 s on UART1 + USB mirror (§6.16) — **only if** options bit6/bit7 or `mavlink_usb_enable` |
| `httpd` (web) | `web_config.c:746` | IDF default (4096) | 5 | config UI + `/api/*`; one task per open socket |
...
Note: `xTaskCreate` / `esp_task_wdt_add` return values are **now checked** (error logged on failure) at `esp_remote_id.c:184,432,683`, `cli.c:415` — tracked in `softwarestatus.md`.
...
## 5) `rid_task` main loop — every phase (`esp_remote_id.c:425-674`)

Period 100 ms (`vTaskDelay` at `:672`), WDT reset at `:673`. `g_running` toggled by `esp_rid_start/stop`.

| Phase | Lines | What happens | If it goes wrong → |
|---|---|---|---|
| A | 427-434 | `active_protocol=UNKNOWN`; `esp_task_wdt_add(NULL)` (return checked, logs on failure) | no WDT coverage if add fails |
| B | 442-445 | brief lock: copy `protocol` + `options` to locals | |
| C | 447-450 | AUTO → `protocol_detect_auto()` (**consumes UART bytes, blocks up to 50 ms**); else `proto = cfg_proto` | AUTO starvation/misclassification (todo) |
| D | 452 | `g_state.active_protocol = proto` (unlocked write) | |
| E | 456-468 | dispatch active parser `*_get()` → UART read + parse + fill `gps_data` | wrong protocol → no data |
| F | 471-474 | if `!have_data && rid_dronecan_is_active()` → `rid_dronecan_get`; on success `active_protocol=NONE` | DroneCAN is non-functional today (see §6.5) |
| G | 478-487 | gate `have_data && lat!=0`; `force_tx = FORCE_ARM_OK && armed`; if `force_tx || fix>=2`: copy `gps_data`→`g_state.gps`, `gps_valid=true`, `last_update_ms` (all under lock) | |
| H | 489-496 | MAVLink only: `mavlink_parser_get_armed`/`get_sysid` → `g_state.mavlink_armed/sysid`; `gps.armed` overwritten | |
| I | 498-515 | identity: MAVLink if fresh & non-empty, else from `g_config` (**unlocked reads** at `:507-514`) | race with web/CLI |
| J | 517-523 | takeoff capture (first `fix>=3` with lat/lon ≠ 0) | |
| K | 526-529 | MSP/NMEA: `altitude_relative = altitude_msl - takeoff_alt` (if captured) | |
| L | 531-548 | operator loc: MAVLink if fresh (<30 s) → `operator_*` + `gps.operator_*`; else from `g_config` | stale MAVLink when proto≠MAVLink |
| M | 550 | give lock | |
| N | 552-566 | if `DONT_SAVE_BASIC_ID`: clear `uas_id`/`uas_id_2`; identity gate: `identity_ready=true` unless (`IDENTITY_READY_GATE` set AND `identity_is_sane(identity, region_rules)` or `position_is_sane()` false); `region_rules = rid_output_region_rules(g_config.region)` | |
| O | 568-589 | **`else if DEMO_MODE`** (only when NO valid GPS this loop): under lock `rid_patrol_tick`, `gps_valid=true`, `had_gps=true`, `active_protocol=NONE`, identity/operator from config; `identity_ready=true` | demo stops as soon as a real fix arrives |
| P | 591-617 | Kalman (`KALMAN && !DEMO`): `rid_kalman_update` from **raw** `gps_data`, then `predict`, then if valid age (<3 s) **unlocked** overwrite of `g_state.gps` (lat/lon/alt/speed/climb/heading), `gps_valid=true`; else if `!had_gps` `gps_valid=false` | race: kalman writes without lock |
| Q | 619-626 | **absolute 10 s timeout** on `last_update_ms` → `gps_valid=false` + WARN | timeout too short/long |
| R | 628-631 | **only if `had_gps`** → `update_transmissions()` (§8); on success `led_status_tx_flash` | bcast_powerup is ineffective when no parser data (see §8) |
| S | 633-642 | LED state (LOCKED>DEMO>GPS_OK>NO_GPS) + `led_status_tick` | |
| T | 645-649 | WS2812 green (GPS) / amber (no GPS) | |
| U | 651-653 | `rid_lighting_set_state(armed, gps_valid)` + `tick` | |
| V | 655-662 | optional RID log line (`PRINT_RID_MAVLINK`) | |
| W | 664-670 | status box every 100 loops, system box every 500 | |
| X | 672-673 | `vTaskDelay(100 ms)` + `esp_task_wdt_reset()` | |
...
- `wifi_tx_transmit` (`:150-175`): builds the UAS pack via **`rid_output_build_uas()`** (`rid_output.c`, §6.7a — region-gated, exclusive standard), then `odid_wifi_build_message_pack_beacon_frame` (RAW 6-byte MAC is fine — the lib uses it as a binary 802.11 MAC), 4-attempt fallback `{AP,STA,AP,STA} × {no-seq,seq}`; returns true on first success.
- `wifi_tx_transmit_nan` (`:178-198`): NAN action frame via `rid_output_build_uas()`, **single** attempt on AP.
- `wifi_tx_reconfigure_ap` (`:131-165`): re-applies AP config + power (called on config change).

### 6.7a Hourglass output hub — `rid_output.c/h` (new)
- The single choke point every transport calls: `rid_output_build_uas(d, gps, identity, cfg)` (`rid_output.c:76-100`) binds the neutral GPS+identity state to the **exclusive standard** selected by `cfg->region` and copies the identity fields that region allows.
- `rid_output_active_standard(cfg)` (`:42`): AUTO→ASTM; EUR/FAA/JPN/SGP/KOR/CAN/AUS/BRA/NZL→ASTM today; CHN→CHN_GB (**no encoder yet** → `rid_output_has_encoder()` false → falls back to ASTM with `ESP_LOGW`).
- `rid_output_region_rules(region)` (`:55`) + static `g_region_rules[]` (`:14-27`): per-region gating — `require_operator_id` only for AUTO/EUR/FAA/CAN/AUS; CHN suppresses OperatorID, SelfID and the second BasicID (GB 42590 constraint).
- Adding a new standard = a new encoder function that plugs in behind `rid_output_build_uas`; adding a new input = nothing here (inputs feed the neutral state).

### 6.8 BLE TX — `ble_tx.c`
- Init (`:124-141`): BT controller (release classic), enable BLE, Bluedroid init+enable.
- `ble_tx_transmit_legacy` (`:146-189`): builds the pack via `rid_output_build_uas()` (§6.7a), one 25 B ODID message per 31 B Service-Data adv (UUID 0xFFFA, app code 0x0D, counter), messages **rotated** per cycle. On S3/C6 (`CONFIG_BT_BLE_50_EXTEND_ADV_EN`): ext-adv **instance 2**, `LEGACY_NONCONN`, 1M PHY; on ESP32 classic: `config_adv_data_raw` + `start_advertising`, `ADV_TYPE_SCAN_IND`.
- `ble_tx_transmit_lr` (`:193-238`): full pack (≤254 B) via `rid_output_build_uas()` on **instance 0** (1M, legacy-compatible) + **instance 1** (Coded PHY). Only if ext-adv enabled.
- `ble_tx_set_power` (`:242-253`): clamps to [-12..9] dBm, level = `(dbm+12)/3`.
...
- Init (`:746-774`): `log_init` installs a `vprintf` hook feeding a 64×240 B log ring; httpd on port 80 (max 16 handlers, LRU purge) + 10 handlers.
...
- `config set` fields: uas_id, operator_id, self_id, wifi_ssid, wifi_password, ua_type, id_type, wifi_channel, mavlink_sysid, bcast_powerup, webserver, lock_level, baud_rate, wifi_power_dbm, wifi/wifi_nan/ble4/ble5 rates+power, operator_lat/lon/alt, start_delay_ms, **region** (`config set region CHN`, `:156-166`). **Cannot** set ws2812, lighting, dronecan, ota_trigger_gpio or auth key.
...
|---|---|---|
| `g_config` | NVS load, web POST, CLI, boot MAC fix | `rid_task` (unlocked: `:316-346,507-514,545-547,559`), TX builders (hub `rid_output_build_uas` via cfg) |
| `g_state.gps` | `rid_task` (locked G/L), Kalman (unlocked `:599-613`), demo (locked) | TX builders (unlocked `:310-358`), web/CLI (locked copy) |
| `g_state.identity` | `rid_task` (locked I) | TX builders, gate (unlocked), web |
| `active_protocol` | `rid_task` (`:452`) | TX gate, web, CLI |
...
## 8) TX decision chain — `update_transmissions()` (`esp_remote_id.c:310-358`)
...
4. per-mode `rate_allowed(last_us, rate_hz)` (`:296-306`, `esp_timer_get_time`, rate ≤ 0 disabled):
...
Each transport passes `&g_config` so the hub (`rid_output_build_uas`, §6.7a) selects the region standard and gates the messages. Each success increments the per-channel counter + `transmissions_count` + LED flash.
...
| BLE4 nothing received | legacy 31 B rotation (`ble_tx.c:146-189`) | `ble4_count` increments? power; ext-adv instance 2 |
...
| Watchdog reset loop | `rid_task` blocked on `g_lock` `portMAX_DELAY` (`esp_remote_id.c:442,483,569`) | who holds the lock (web/CLI/NVS)? |
